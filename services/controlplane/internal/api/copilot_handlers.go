...
const actionCatalogDesc = "Catalog (kind -> target kinds, params): scale -> Deployment,StatefulSet {replicas:0..50}; rollout_restart -> Deployment,StatefulSet,DaemonSet; rollout_undo|rollout_pause|rollout_resume -> Deployment; set_image -> Deployment,StatefulSet,DaemonSet {image}; delete_pod -> Pod; hpa_set -> HorizontalPodAutoscaler {minReplicas?,maxReplicas:1..200}; cronjob_trigger|cronjob_suspend|cronjob_resume -> CronJob; cleanup_pods -> Namespace (targetName = namespace); cordon|uncordon|drain -> Node; node_taint -> Node {key,effect}; node_untaint -> Node {key}; set_resources -> Deployment,StatefulSet,DaemonSet {container?,requestsCpu?,requestsMemory?,limitsCpu?,limitsMemory?}; set_env -> Deployment,StatefulSet,DaemonSet {container?,name,value,remove?}; rollout_to_revision -> Deployment {revision}; rollout_history -> Deployment,StatefulSet (READ) {limit?}; statefulset_partition -> StatefulSet {partition}; hpa_toggle -> HorizontalPodAutoscaler {enabled}; annotate -> most kinds {key,value,remove?}; set_label -> Node,Namespace {key,value,remove?}; patch_configmap -> ConfigMap {key,value,remove?} (needs rollout_restart to take effect); evict_pod -> Pod {gracePeriodSeconds?} (PDB-aware); cleanup_jobs -> Namespace {states?,olderThanHours?}; drain_preview -> Node (READ, blast-radius); exec_readonly -> Pod {command:argv array (cat|ls|head|tail|df|du|stat|wc|env|id|uname|find, no shell), container?} — read a file INSIDE a container (e.g. an error log only written to disk); patch_secret -> Secret {key, value, remove?}; create_configmap -> ConfigMap {data:{k:v,...}}; delete_configmap -> ConfigMap (snapshot kept for restore); pdb_set -> PodDisruptionBudget {minAvailable? XOR maxUnavailable?, int or percent string}; patch_resource -> Service,Ingress,NetworkPolicy,PodDisruptionBudget {patch: JSON merge patch} (generic spec edit, snapshot + restorable); pvc_expand -> PersistentVolumeClaim {size, only GROW}; delete_job -> Job; restore_resource -> {snapshot} (re-apply a before-snapshot from a prior action).; pod_logs -> Pod (READ) {container?, previous?, tailLines?} — kubectl logs incl. the PREVIOUS crashed container; list_events -> Namespace (READ) {warningsOnly?, limit?}; net_probe -> Namespace (READ) {mode:http|tcp|dns|tls, target, method?} — reachability/TLS-expiry/DNS check from inside the cluster; patch_resource also accepts params.apiVersion + params.resource to patch ANY CRD (Flux/Argo suspend, cert-manager renewal, CNPG failover) — RBAC/webhook/CRD-definition groups are denied; exec_readonly also takes {retry:true} to catch a CrashLoop pod's brief run window; run_debug_pod -> Namespace {image, command:argv, pvcName? (mounted read-only at /pvc), nodeName?, memory?, cpu?} — an EPHEMERAL probe pod (no SA token, auto-deleted, logs are the result): use it for image bisecting (run a candidate image with its own binary check), HTTP probes from inside the cluster (curlimages/curl + curl -sS http://svc:port/health), DNS checks (busybox nslookup), PVC data inspection (mount the PVC read-only and ls/cat), or testing a specific node (nodeName). These generic primitives compose to cover almost any incident. For Node/Namespace targets set targetNamespace to '-'. EVERY kind additionally accepts params.timeoutSeconds (10..1800, default 180) — raise it for slow rollouts/drains, lower it for quick probes; on timeout the engine rolls back and reports the last observed state. Waits fail FAST with the real reason when a pod is stuck in CrashLoopBackOff/ImagePullBackOff instead of sitting out the timeout. Propose ONE at a time; never invent a kind or param outside this list."
...
func copilotTools() []copilotTool {
	...
	tools := []copilotTool{
		...
		{"get_resource", "Read the FULL live spec of one Kubernetes object as YAML (like kubectl get -o yaml, managedFields/status stripped): ConfigMap (full data!), workloads, Service, Ingress, NetworkPolicy, PDB, HPA, CronJob, Job, PVC, Node, Namespace, quotas — AND any CRD (cert-manager Certificate, ArgoCD Application, CNPG Cluster, Istio VirtualService, …) by also passing apiVersion + resource (plural). Secrets show keys + sha256 hashes only. THE tool to inspect configuration.", o(map[string]any{"namespace": sp("namespace ('-' for cluster-scoped)"), "kind": sp("exact resource kind, e.g. ConfigMap or Certificate"), "name": sp("object name"), "apiVersion": sp("for CRDs: group/version, e.g. cert-manager.io/v1"), "resource": sp("for CRDs: plural resource name, e.g. certificates")}, "kind", "name")},
		{"describe_resource", "kubectl-describe-like view of one object: status, conditions, and its recent events. Works for any CRD too (pass apiVersion + resource plural). Use when you need the CURRENT STATE + why (conditions/events), not the spec.", o(map[string]any{"namespace": sp("namespace ('-' for cluster-scoped)"), "kind": sp("exact resource kind"), "name": sp("object name"), "apiVersion": sp("for CRDs: group/version"), "resource": sp("for CRDs: plural resource name")}, "kind", "name")},
		{"pod_logs", "Read a pod's logs DIRECTLY from the kubelet (kubectl logs) — including the PREVIOUS crashed container (previous=true, THE way to read a crash reason) and a specific container. Independent of the log pipeline; use when collected logs are missing or you need the pre-crash tail.", o(map[string]any{"namespace": sp("namespace"), "pod": sp("exact pod name"), "container": sp("container name (default: first)"), "previous": bp("read the PREVIOUS (crashed) container's log"), "tailLines": ip("lines from the end (default 200, max 500)")}, "namespace", "pod")},
		{"list_events", "ALL recent events of a namespace (not just one object's) sorted newest-first — 'what happened here around 17:30?'. warningsOnly filters to Warning events.", o(map[string]any{"namespace": sp("namespace (omit for all namespaces)"), "warningsOnly": bp("only Warning events (default false)"), "limit": ip("max events (default 40, max 100)")})},
		{"net_probe", "Probe reachability FROM INSIDE the cluster (the agent runs the check, no probe pod): mode=http (GET/HEAD an in-cluster URL → status + latency + body snippet), tcp (host:port open? — is the DB reachable?), dns (name resolvable? — service discovery broken?), tls (handshake + certificate chain with DAYS-UNTIL-EXPIRY — the classic 'cert expired' incident). Fast and precise.", o(map[string]any{"mode": sp("http | tcp | dns | tls"), "target": sp("http: an http(s):// URL; tcp/tls: host:port; dns: a hostname"), "method": sp("http only: GET (default) or HEAD")}, "mode", "target")},
...
func (s *Server) execCopilotTool(ctx context.Context, scope, org, cluster, cookie, name string, args map[string]any) (string, error) {
	...
	case "get_resource", "describe_resource":
		...
		body := map[string]any{"kind": name, "targetNamespace": ns, "targetKind": kind, "targetName": getStr("name")}
		if av, res := getStr("apiVersion"), getStr("resource"); av != "" && res != "" {
			body["params"] = map[string]any{"apiVersion": av, "resource": res}
		}
		return s.runReadAction(ctx, post, get, body)
	case "pod_logs":
		params := map[string]any{}
		if c := getStr("container"); c != "" {
			params["container"] = c
		}
		if b, ok := args["previous"].(bool); ok && b {
			params["previous"] = true
		}
		if n := getInt("tailLines", 0); n > 0 {
			params["tailLines"] = n
		}
		return s.runReadAction(ctx, post, get, map[string]any{"kind": "pod_logs", "targetNamespace": scopedNS("namespace"), "targetKind": "Pod", "targetName": getStr("pod"), "params": params})
	case "net_probe":
		return s.runReadAction(ctx, post, get, map[string]any{"kind": "net_probe", "targetNamespace": scopedNS("namespace"), "targetKind": "Namespace", "targetName": orDefStr(scopedNS("namespace"), "-"), "params": map[string]any{"mode": getStr("mode"), "target": getStr("target"), "method": getStr("method")}})
	case "list_events":
		ns := scopedNS("namespace")
		params := map[string]any{}
		if b, ok := args["warningsOnly"].(bool); ok && b {
			params["warningsOnly"] = true
		}
		if n := getInt("limit", 0); n > 0 {
			params["limit"] = n
		}
		return s.runReadAction(ctx, post, get, map[string]any{"kind": "list_events", "targetNamespace": "-", "targetKind": "Namespace", "targetName": orDefStr(ns, "-"), "params": params})
	case "get_secret":
