...
func masterTools() []copilotTool {
	...
	tools := []copilotTool{
		...
		{"propose_script", "LAST RESORT: author a Starlark action script for a remedy no catalog action covers. It is validated, then proposed to the human as a DESTRUCTIVE action (source shown, arm-to-fire) — never executed silently. The script runs hermetically with k8s.* builtins and automatic LIFO undo. Use only when the catalog genuinely cannot express the fix.", o(map[string]any{
			...
			"source":         sp("the Starlark source. Every mutation auto-captures a snapshot first, so the whole run auto-rolls-back on any failure/cancel and stays revertible. Globals: step(name), report(detail), fail(msg), sleep(s), snapshot(ns,kind,name), args dict, json, net_probe(mode,target,method?), wait_rollout(ns,kind,name,timeout?), wait_ready(...). Reads: k8s.get(api_version,kind,ns,name,resource?) → full object (CRDs via resource=plural, Secrets redacted), k8s.list(api_version,kind,ns,selector?), k8s.status(ns,kind,name) → {desired,ready,updated,available}, k8s.pods(ns,selector?), k8s.events(ns,name?). Mutators (auto-snapshot): k8s.patch(ns,kind,name,patch,strategic?), k8s.set_field(ns,kind,name,path,value), k8s.set_fields(ns,kind,name,entries), k8s.scale(ns,kind,name,replicas), k8s.patch_configmap(ns,name,key,value), k8s.create(manifest), k8s.delete(ns,kind,name), k8s.apply(manifest). Host: k8s.exec(ns,name,command,container?,retry?), k8s.logs(ns,name,container?,previous?,tail?), k8s.evict(ns,name,grace_seconds?), k8s.node_pods(node), k8s.pod_status(ns,name). Mutations are bound to a whitelist of kinds (RBAC/webhook/CRD-definition/Node-privileged edits are denied)."),
